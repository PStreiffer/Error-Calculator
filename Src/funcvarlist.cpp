#include "funcvarlist.h"
    ...
    createdfunc::createdfunc(string namein, int argnumin, function<errval(vector<errval>)> functionin){
        name = namein;
        argnum = argnumin;
        namedfunc = functionin;
    }
    createdfunc::createdfunc(string namein = "",int argnumin = 0, function<errval()> functionin = [](){return 0;}){
        name = namein;
        argnum = argnumin;
        namedfunc = [functionin](vector<errval> a){return functionin();};
    }
    errval createdfunc::operator()(vector<errval> input) {
...
createdvar::createdvar(char namein = ' ', errval valin = 0){
        name = namein;
        value = valin;
    }
