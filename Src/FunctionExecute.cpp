#include "FunctionExecute.h"
    ...
    evalresult::evalresult(vector<errval> input){
        resulttype = 'E';
        errvalist = input;
        erval = input[0];
    }
    evalresult::evalresult(errval input){
        resulttype = 'e';
        erval = input;
        errvalist = {input};
    }
    evalresult::evalresult(createdfunc input){
        resulttype = 'f';
        funval = input;
        funvalist = {input};
    }
    evalresult::evalresult(createdvar input){
        resulttype = 'v';
        varval = input;
    }
    evalresult::evalresult(vector<createdfunc> input){
        resulttype = 'F';
        funvalist = input;
        funval = funvalist[0];
    }
...
evalresult functioneval(vector<vector<double>> arglist, bool funcdef, int funcvarstartin){
    errcorrection = false;
